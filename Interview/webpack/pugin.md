webpack 整个生命周期暴露了很多钩子函数，插件可以理解为基于这些钩子的自定义函数。在其原型上暴露了 apply 方法，如果特定钩子函数触发，就会调用插件去干我们响应干的事情。如`imagemin-webpack-plugin` 在 emit 阶段，把生成的资源，进行拦截、压缩图片、替换图片资源。达到压缩的目的
